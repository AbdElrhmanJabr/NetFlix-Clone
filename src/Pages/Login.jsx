...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage = error.message.split("auth/");
      setError(errorMessage[1].split(")."));
    }
  };
  console.log(error);
  return (
      ...
      <div>
        <div className="w-full h-[650px] relative flex items-center justify-center">
          ...
          <div className="absolute bg-black/70 w-[350px] md:w-[500px] px-16 py-4 flex gap-3  flex-col h-[400px] lg:h-[500px] z-50 text-white md:p-16">
            ...
            {error && <p className=" capitalize">{error}</p>}
            <form
            ...
            <p className="text-write  text-gray-400">
              New To NETFLIX ?!
              <Link to={"/signUp"} className="text-gray-200 font-bold ms-1">
                Sign Up
              </Link>
            </p>
          </div>
